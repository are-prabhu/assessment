...
def findroute(host, timeout, maxHops):
    # timeout=1 means: If one second goes by without a reply from the server,
    # the client assumes that either the client's ping or the server's pong is
    # lost
    dest = socket.gethostbyname(host)
    print("finding latency to %s (%s), %d hops max" % (host, dest, maxHops))
    print('\thops name/ip\t\t\tip\t\t\t average latency')
...
def run():
    ...
    Usage:
        custom_mtr.py [options] <host>

    Options:
        -c,--count=packet_count Specifie number of packets to capture.
    """
    import docopt
    import textwrap
    import os
